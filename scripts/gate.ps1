...
Invoke-Gate 'php-coverage' 'docker' { & (Join-Path $root 'scripts/coverage.ps1') }
...
if ($IncludeBrowser) {
    Invoke-Gate 'browser-portable' 'docker' { & (Join-Path $PSScriptRoot 'browser-gate.ps1') -Task playwright }
}
