...
Invoke-Gate 'wordpress-integration' 'docker' { docker run --rm --user 33:33 --network labm_default --entrypoint php -e WORDPRESS_DB_HOST=db:3306 -e WORDPRESS_DB_NAME=labm_demo -e WORDPRESS_DB_USER=labm_demo -e WORDPRESS_DB_PASSWORD=demo_password_change_me -e WP_TESTS_RUNTIME_ROOT=/wordpress -v "${root}:/app" -v labm_composer_vendor:/app/vendor -v labm_wordpress_core:/wordpress -v labm_wordpress_uploads:/wordpress/wp-content/uploads -v "${root}/wp-content/themes/labm:/wordpress/wp-content/themes/labm:ro" -v "${root}/wp-content/plugins/labm-core:/wordpress/wp-content/plugins/labm-core:ro" -w /app wordpress:cli-2.11.0-php8.3 /app/vendor/bin/phpunit -c phpunit.integration.xml.dist }
