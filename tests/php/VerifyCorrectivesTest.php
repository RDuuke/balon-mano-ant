...
final class VerifyCorrectivesTest extends TestCase {
	public function test_publicacion_autorizada_y_contenido_incompleto(): void {
		...
		$error = labm_core_validate_publishable(
			'labm_actualidad',
			array(
				'post_title'   => '',
				'post_content' => '',
	...
	public function test_presentacion_generica_e_identificador_invalido(): void {
		...
		$post = wp_insert_post(
			array(
				...
				'post_status'  => 'publish',
				'post_title'   => 'SelecciÃ³n genÃ©rica',
				'post_content' => 'Contenido usable',
			)
		);
	...
	public function test_archivo_exclusivo_sigue_politica_explicita(): void {
		...
		$document   = wp_insert_post(
			array(
				...
				'post_status' => 'draft',
				'post_title'  => 'Documento exclusivo',
			)
		);
	...
	public function test_validacion_accesible_y_error_entrega_sin_datos_personales(): void {
		$attrs = labm_core_contact_error_attributes(
			array(
				'correo'  => 'InvÃ¡lido',
				'mensaje' => 'Obligatorio',
			)
		...
		$logged = array();
		add_action(
			'labm_core_contact_delivery_failed',
			static function ( $context ) use ( &$logged ) {
				$logged = $context;
			}
		);
		add_filter( 'pre_wp_mail', '__return_false' );
		$result = labm_core_process_contact(
			array(
				'nombre'    => 'Ana',
				'apellidos' => 'PÃ©rez',
	...
	public function test_navegacion_completa_portada_completa_y_seccion_opcional(): void {
		...
		self::assertNotContains( 'modalidades', labm_theme_home_sections() );
		self::assertTrue( post_type_exists( 'labm_seleccion' ) );
