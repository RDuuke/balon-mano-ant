...
- `FTP_SERVER`, `FTP_USERNAME` y `FTP_PASSWORD`: cuenta FTP/SFTP con acceso a `/public_html/`.
...
3. Haga merge a `main`. Tras `quality`, el job `deploy-code` actualiza únicamente `wp-content/themes/labm/` y `wp-content/plugins/labm-core/`, sin limpieza remota.
